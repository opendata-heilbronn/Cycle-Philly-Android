...
public class TripUploader extends AsyncTask<Long, Integer, Boolean> {
   ...
   /*     boolean result = false;
	...
	protected void onPreExecute() {
		Toast.makeText(mCtx.getApplicationContext(), "Submitting trip. Thanks for using Cycle Philly!",
		...
		try {
			if (result) {
				...
				Toast.makeText(mCtx.getApplicationContext(),
						"Cycle Philly couldn't upload the trip, and will retry when your next trip is completed.",
						Toast.LENGTH_LONG).show();
			}
		}
		catch (Exception e) {
